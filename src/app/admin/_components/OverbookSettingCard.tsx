...
export function OverbookSettingCard() {
  ...
  return (
    <Card className="p-4">
      <SectionTitle hint="모든 주점의 과팅(매칭) 예약에 동일하게 적용">
        오버부킹(대기 접수) 허용 수
      </SectionTitle>
      <p className="mt-2 text-xs text-neutral-500 dark:text-neutral-400">
        과팅(매칭) 테이블 정원마다, 등록된 테이블 수를 넘겨서 성별별로 몇 팀까지 더
        접수(대기)를 받을지 정해요. 일반 예약은 정원이 차면 바로 마감되고 대기를 받지
        않아 이 설정과 무관합니다. 승인(확정) 시점에는 이 여유분과 무관하게 실제 테이블
        수만 그대로 적용돼요.
      </p>
